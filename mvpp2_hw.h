...
static inline void mvpp2_shared_thread_interrupts_mask(
		struct mvpp2_port *port)
{
	...
	for (i = 0; i < port->num_qvector; i++) {
		if (q_vec[i].qv_type == MVPP2_SHARED)
			mvpp22_thread_write(&port->priv->hw,
...
static inline void mvpp2_shared_thread_interrupts_unmask(
		struct mvpp2_port *port)
{
	struct queue_vector *q_vec = &port->q_vector[0];
	int i;
...
static inline struct mvpp2_rx_queue *mvpp2_get_rx_queue(
		struct mvpp2_port *port, u32 cause)
{
...
static inline struct mvpp2_tx_queue *mvpp2_get_tx_queue(
		struct mvpp2_port *port, u32 cause)
{
			...
			u32 pool, dma_addr_t pool_addr, int size)
{
	...
	mvpp2_write(hw, MVPP2_BM_POOL_BASE_ADDR_REG(pool),
 ...
 * it is a kmem_cache address (YuvalC).*/
	...
	mvpp2_write(hw, MVPP2_BM_VIRT_RLS_REG,
		lower_32_bits((uintptr_t)buf_virt_addr));
	mvpp2_write(hw, MVPP2_BM_PHY_RLS_REG(pool),
...
static inline void mvpp2_bm_pool_mc_put(struct mvpp2_port *port, int pool,
		u32 buf_phys_addr, u32 buf_virt_addr, int mc_id)
{
	u32 val = 0;
...
static inline struct sk_buff *mvpp21_rxdesc_cookie_get(
		struct mvpp2_rx_desc *rx_desc)
{
	return((struct sk_buff *)((uintptr_t)rx_desc->u.pp21.buf_cookie));
}

static inline dma_addr_t mvpp21_rxdesc_phys_addr_get(
		struct mvpp2_rx_desc *rx_desc)
{
...
static inline struct sk_buff *mvpp22_rxdesc_cookie_get(
		struct mvpp2_rx_desc *rx_desc)
{
...
static inline dma_addr_t mvpp22_rxdesc_phys_addr_get(
		struct mvpp2_rx_desc *rx_desc)
{
	return((dma_addr_t)
...
static inline struct sk_buff *mvpp21_txdesc_cookie_get(
		struct mvpp2_tx_desc *tx_desc)
{
	return((struct sk_buff *)((uintptr_t)tx_desc->u.pp21.buf_cookie));
}

static inline dma_addr_t mvpp21_txdesc_phys_addr_get(
		struct mvpp2_tx_desc *tx_desc)
{
	return (dma_addr_t)tx_desc->u.pp21.buf_phys_addr;
}

static inline struct sk_buff *mvpp22_txdesc_cookie_get(
		struct mvpp2_tx_desc *tx_desc)
{
	return((struct sk_buff *)((uintptr_t)
...
static inline dma_addr_t mvpp22_txdesc_phys_addr_get(
		struct mvpp2_tx_desc *tx_desc)
{
	return((dma_addr_t)
...
int mvpp2_prs_sw_sram_shift_set(struct mvpp2_prs_entry *pe, int shift,
					unsigned int op);
...
int mvpp2_prs_sw_sram_offset_get(struct mvpp2_prs_entry *pe,
		unsigned int *type, int *offset, unsigned int *op);
void mvpp2_prs_hw_port_init(struct mvpp2_hw *hw, int port,
...
int mvpp2_cls_hw_lkp_write(struct mvpp2_hw *hw, int lkpid, int way,
				struct mvpp2_cls_lookup_entry *fe);
...
int mvpp2_cls_c2_qos_hw_read(struct mvpp2_hw *hw, int tbl_id, int tbl_sel,
			int tbl_line, struct mvpp2_cls_c2_qos_entry *qos);
...
int mvpp2_cls_c2_qos_tbl_set(struct mvpp2_cls_c2_entry *c2, int tbl_id,
				int tbl_sel);
...
u8 mvpp2_cls_c2_pbit_tbl_queue_get(struct mvpp2_hw *hw, u8 tbl_id,
					u8 tbl_line);
void mvpp2_cls_c2_pbit_tbl_queue_set(struct mvpp2_hw *hw, u8 tbl_id,
...
int mvpp2_cls_c2_prio_set(struct mvpp2_cls_c2_entry *c2, int cmd,
				int prio, int from);
int mvpp2_cls_c2_dscp_set(struct mvpp2_cls_c2_entry *c2, int cmd,
				int dscp, int from);
int mvpp2_cls_c2_queue_low_set(struct mvpp2_cls_c2_entry *c2, int cmd,
				int queue, int from);
int mvpp2_cls_c2_queue_high_set(struct mvpp2_cls_c2_entry *c2, int cmd,
				int queue, int from);
