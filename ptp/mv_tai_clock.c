...
#ifdef ARMADA_390
MODULE_DEVICE_TABLE(of, mv_pp3_shared_match);/* vs mv_pp2x_match_tbl */
...
#else
#define MV_PTP_EVENT_MODULE_ENABLE
int __init mv_ptp_event_init_module(void)
{
#ifdef MV_PTP_EVENT_MODULE_ENABLE
	if (platform_driver_register(&ptp_event_driver))
		pr_err("%s: Can't register %s\n", PTP_TAI_PRT_STR, MV_PTP_EVENT_DEV_DTB_STRING);
#else
	(void)ptp_event_driver;
#endif
	return 0;
}
...
{
#ifdef MV_PTP_EVENT_MODULE_ENABLE
	platform_driver_unregister(&ptp_event_driver);
#endif
	return 0;
}
MODULE_DESCRIPTION("Marvell PPv2 PTP Pin Event driver - www.marvell.com");
#endif
